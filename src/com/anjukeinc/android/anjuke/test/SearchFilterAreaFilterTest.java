...
public class SearchFilterAreaFilterTest extends ActivityInstrumentationTestCase2<WelcomeActivity>{
	private Solo solo;
	private int count = 3;
	private String bureau = "浦东";
	private String area = "金桥";
	private String testName = "安居客Android客户端V2.3搜索页区域筛选用例";



	//最先起到那个页面
	public SearchFilterAreaFilterTest(){
	...
	public void testSearchFilterArea(){
		try {
			...
			Action.scroolUpPage(solo, "筛选不关联");
			...
			Report.writeHTMLLog("搜索页区域搜索", "筛选信息为："+searchTip,Report.DONE ,"");

			Boolean flag = Action.assertFilter(solo, "区域选择筛选信息匹配", searchTip, bureau,area);

			if(flag){
				Report.writeHTMLLog("搜索页区域搜索", "区域筛选找房条件匹配成功", Report.PASS, "");				
			}else{
				String ps = Action.screenShot(solo);
				Report.writeHTMLLog("搜索页区域搜索", "区域筛选条件匹配不成功,用例执行失败", Report.FAIL, ps);				
				fail("区域选择筛选信息匹配失败");       
			}
