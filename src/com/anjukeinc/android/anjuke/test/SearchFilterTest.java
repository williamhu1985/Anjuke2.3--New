...
/*
 * @author:WilliamHu
 * #1.搜索页设置完筛选条件，包括售价、户型、面积、房龄
 * #2.检查头部灰色信息栏筛选条件是否符合
 * #3.检查房源列表页信息是否符合筛选条件
 */

public class SearchFilterTest extends ActivityInstrumentationTestCase2<WelcomeActivity>{
	private Solo solo;
	private int count = 5;
	private int interceptor = 0;
	private String testName = "安居客Android客户端V2.3搜索页筛选房源用例";
    private Boolean scroolFlag = true;
	...
	public void testSearchFilter(){
		try {
			...
			Action.scroolUpPage(solo, "附近页筛选房源");	
			
			solo.clickOnText("静安");
			solo.clickOnText("南京西路");
			...
			Report.writeHTMLLog("搜索页筛选房源", "Price:"+ price+" Chamber:"+chamber + " Acreage:"+acreage+" Houseage:"+houseage +"---->", Report.DONE, "");

			solo.clickOnText("完成");
			solo.waitForText("筛选");
			String  searchTip= (String)((TextView)solo.getView(R.id.activity_near_property_list_tv_conditons)).getText();
			Report.writeHTMLLog("附近房源筛选房源","serachTip:"+searchTip, Report.DONE, "");
			Boolean flag = Action.assertFilter(solo, "筛选条件和筛选信息栏信息比较", searchTip, price,chamber,acreage,houseage);

			if(flag){
				Report.writeHTMLLog("设置筛选找房条件", "筛选找房条件匹配成功---->", Report.PASS, "");				
			}else{
				String ps = Action.screenShot(solo);
				Report.writeHTMLLog("设置筛选找房条件", "筛选条件匹配不成功，不是预期结果,用例执行失败---->", Report.FAIL, ps);				
				fail("筛选找房条件设置失败---->");       
			}
			...
			if(price.indexOf("-")>0){
				...
				Report.writeHTMLLog("设置筛选找房条件", "去掉空格的价格条件为:"+price, Report.DONE, "");				
			}
			//点击显示的房源列表中的第一套房子
			Action.wait(solo, R.id.view_list_item_tv_title, 3, "等待页面列表加载成功");
			solo.clickOnView(solo.getView(R.id.view_list_item_tv_title));
			//等待房源详细信息显示完全
			solo.waitForText("收藏");
			Report.writeHTMLLog("设置筛选找房条件", "房源信息详细页加载成功", Report.DONE, "");				
		
			//获取房源详细页中的售价.户型.面积.房龄
			String propPrice = ((TextView)solo.getView(R.id.prop_total_price)).getText().toString();
			Report.writeHTMLLog("设置筛选找房条件", "房源详细信息页价格---->"+propPrice, Report.DONE, "");	
			String propChamber = ((TextView)solo.getView(R.id.prop_type)).getText().toString();
			Report.writeHTMLLog("设置筛选找房条件", "房源详细信息页室为---->"+propChamber, Report.DONE, "");	
			String propAcreage = ((TextView)solo.getView(R.id.prop_area)).getText().toString();
			Report.writeHTMLLog("设置筛选找房条件", "房源详细信息页面积为---->"+propAcreage, Report.DONE, "");	
			String propHouseage = ((TextView)solo.getView(R.id.prop_build)).getText().toString();
			Report.writeHTMLLog("设置筛选找房条件", "房源详细信息页年代为---->"+propHouseage, Report.DONE, "");	
...
//			System.out.println("房源详细信息页价格1---->"+propPrice);
			...
			Report.writeHTMLLog("设置筛选找房条件", "房源详细信息页价格为---->"+propPrice, Report.DONE, "");	
			...
			Report.writeHTMLLog("设置筛选找房条件", "房源详细信息页室数为---->"+propChamber, Report.DONE, "");	
			...
			Report.writeHTMLLog("设置筛选找房条件","房源详细信息页年代为--->"+propHouseage, Report.DONE, "");		
			Report.writeHTMLLog("设置筛选找房条件","房源详细信息页信息为--->"+ propPrice +" " + propChamber + " " + propAcreage + " " + propHouseage, Report.DONE, "");		
			...
			Report.writeHTMLLog("设置筛选找房条件", "Pass", Report.PASS, "");
